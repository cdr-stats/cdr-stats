...
from cdr.models import FreeswitchCDR as CDR
...
class FreeswitchBackend(CDRBackend):
    
    backend_name = "Freeswitch"
    
    def show_graph_by_day(self, request):
        kwargs = {}
        tday = datetime.today()
        ...
        #calls_in_day = CDR.objects.filter(**kwargs).extra(select={'hour': "django_date_trunc('hour', %s.calldate)" % CDR._meta.db_table}).values('hour').annotate(Count('calldate'))#.order_by('-calldate')#
            ...
            return render_to_response(get_cdr_backend_template(self.backend_name, 'show_graph_by_day.html'), variables,
                   context_instance = RequestContext(request))
                   

    def show_cdr(self, request): 
        ...
        return render_to_response(get_cdr_backend_template(self.backend_name, 'show_jqgrid.html'), variables,
           context_instance = RequestContext(request))                   
    ...
    def show_graph_by_month(self, request):
        ...
        if kwargs:
            ...
            return render_to_response(get_cdr_backend_template(self.backend_name, 'show_graph_by_month.html'), variables,
               context_instance = RequestContext(request))


    def show_graph_by_hour(self, request):
        ...
        if kwargs:
            ...
            return render_to_response(get_cdr_backend_template(self.backend_name, 'show_graph_by_hour.html'), variables,
               context_instance = RequestContext(request))
        

    def show_concurrent_calls(self, request):
        kwargs = {}
        graph_view = '1'
        ...
        return render_to_response(get_cdr_backend_template(self.backend_name, 'show_graph_concurrent_calls.html'), variables,
           context_instance = RequestContext(request))


    def show_global_report(self, request):
        template = get_cdr_backend_template(self.backend_name, 'show_global_report.html')
        kwargs = {}
        total_data = []
    ...
    def show_dashboard(self, request):
        template = get_cdr_backend_template(self.backend_name, 'show_dashboard.html')
        debug = []
        kwargs = {}
        calls = ""
